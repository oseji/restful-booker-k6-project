...
**The suite cleans up after itself, and checks that it did.** Every booking created during a run is deleted before that iteration ends — in the lifecycle test as the subject of the journey, and in the creation test as an explicit teardown step. Both assert the cleanup rather than firing it and hoping: the lifecycle test re-fetches the booking and expects a `404`, and the creation test checks its teardown returned the `201` that means success. Cleanup that quietly stops working is the worst case here — bookings would pile up on someone else's server while the run still reported green. The one accepted exception is a `DELETE` that fails under load (see quirk 6): those bookings do survive the run, and the suite deliberately doesn't retry, since a silent retry would hide exactly the failure the test exists to surface.

**The chain bails out instead of cascading.** If auth returns no usable token, or create returns no booking id, the iteration returns early. Without those guards a failed create would send every downstream request to `/booking/undefined`, burying the one real failure under a pile of meaningless derived ones. Crucially, the guards and the checks test the same condition — `isValidToken` is defined once in `utils/getAuthResponse.js` and used by both — so a skipped iteration always coincides with a failed check. If the two could disagree, a run could quietly skip most of its work and still report everything it did run as passing.
